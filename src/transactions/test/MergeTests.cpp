...
TEST_CASE_VERSIONS("merge", "[tx][merge]")
{
        ...
        SECTION("with trustline")
        {
            ...
            SECTION("account has offer")
            {
                for_all_versions(*app, [&] {
                    ...
                    for (int i = 0; i < 4; i++)
                    {
                        a1.manageOffer(0, gram, curIssued, somePrice, 100);
                    }
                    REQUIRE_THROWS_AS(a1.merge(b1),
                                      ex_ACCOUNT_MERGE_HAS_SUB_ENTRIES);
                });
    ...
    SECTION("account has only base reserve + one µg")
    {
        auto mergeFrom = root.create(
    ...
    SECTION("account has only base reserve + one operation fee - one µg")
    {
        auto mergeFrom = root.create(
            "merge-from",
    ...
    SECTION("account has only base reserve + two operation fees - one µg")
    {
        auto mergeFrom = root.create(
            "merge-from",
