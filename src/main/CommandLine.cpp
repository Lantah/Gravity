...
CommandLine::ConfigOption::getConfig(bool logToFile) const
{
    Config config;
    auto configFile =
        mConfigFile.empty() ? std::string{"gravity.cfg"} : mConfigFile;
...
runForceSCP(CommandLineArgs const& args)
{
    ...
    auto resetOption = clara::Opt{reset}["--reset"](
        "reset force SCP flag, so next time gravity "
        ...
        [&] {
            ...
            try
            {
                ...
                if (cfg.MANUAL_CLOSE)
                {
                    ...
                    {
                        LOG_ERROR(DEFAULT_LOG, "Starting gravity in "
...
{
    std::cout << GRAVITY_VERSION << std::endl;
#ifdef ENABLE_NEXT_PROTOCOL_VERSION_UNSAFE_FOR_PRODUCTION
...
handleCommandLine(int argc, char* const* argv)
{
    auto commandLine = CommandLine{
        {{"catchup",
         ...
         {"http-command", "send a command to local gravity",
          runHttpCommand},
