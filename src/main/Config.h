...
class Config : public std::enable_shared_from_this<Config>
{
  ...
  public:
    ...
    // The default way gravity starts is to load the state from disk and
    ...
    // captive core. This is useful for testing how gravity buffers ledgers
    // during captive core fast restart.
    std::chrono::seconds ARTIFICIALLY_DELAY_BUCKET_APPLICATION_FOR_TESTING;

    // A config parameter that forces gravity to sleep every time a task is
    ...
    uint32_t TESTING_UPGRADE_DESIRED_FEE; // in µg
    uint32_t TESTING_UPGRADE_RESERVE;     // in µg
