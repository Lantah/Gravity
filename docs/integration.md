...
gravity is at the bottom of the Lantah stack, many other services can be
 built on top of it.

 Example of such services include OrbitR, a service to assist with
 submitting transactions to the network as well as exposing a data model to
 query ledger history.

 In the rest of this document we'll use OrbitR to better illustrate data flows.

# Integration points: using data generated by gravity

gravity generates several types of data that can be used by applications, depending on use cases.
...
* core's SQL database (latest only, updated every ledger)
  * in the case of captive-core (enabled via the `--in-memory` command line option) the ledger is maintained within the gravity process and ledger-state need to be tracked as it changes via "meta" updates.
...
As part of this transition, gravity records:
...
In captive-core mode, this entire per-ledger transition dataset is emitted by gravity over a (optionally named) pipe, as an xdr encoded `LedgerCloseMeta` object.
...
* for more demanding applications, custom views can be built using a pub/sub model where data from gravity is published and consumed by many different services
...
A high resolution custom view allows to build the most sophisticated data model and derive complex data from the Lantah Network.

With such view, systems like OrbitR can:
...
When starting fresh, before being able to use ledger changes, the processor (OrbitR) needs to initialize its initial state.
...
Right now, gravity uses SQL to store the current state of the ledger.
...
* developers of OrbitR have to
  *  keep track of changes in how gravity is storing and updating data in its database
  *  be extremely careful when performing requests as to avoid
     *  inconsistent queries
        *  gravity may change ledger between or while OrbitR is making queries
        *  multiple gravity instances may not be in perfect sync with each other
     *  performance problems with poorly designed queries (non-indexed, etc).
* operationally, this creates coupling between gravity and OrbitR:
  * a traffic spike in OrbitR may cause enough stress on gravity that it loses sync
  * it's hard to "scale out" OrbitR

### Additional reading

An overview of how the data flows within gravity is [this data flow presentation](software/core-data-flow.pdf)
...
properly registered by updating its cursor so that gravity can safely
garbage collect data no longer needed.
By default gravity only keeps track of its internal state and will
...
gravity exposes a simple HTTP based API in particular the `tx` endpoint is used to submit transactions.
