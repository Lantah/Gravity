...
gramr is at the bottom of the Stellar stack, many other services can be
 built on top of it.
...
# Integration points: using data generated by gramr

gramr generates several types of data that can be used by applications, depending on use cases.
...
* core's SQL database (latest only, updated every ledger)
  * in the case of captive-core (enabled via the `--in-memory` command line option) the ledger is maintained within the gramr process and ledger-state need to be tracked as it changes via "meta" updates.
...
As part of this transition, gramr records:
...
In captive-core mode, this entire per-ledger transition dataset is emitted by gramr over a (optionally named) pipe, as an xdr encoded `LedgerCloseMeta` object.
...
* for more demanding applications, custom views can be built using a pub/sub model where data from gramr is published and consumed by many different services
...
Right now, gramr uses SQL to store the current state of the ledger.
...
* developers of Horizon have to
  *  keep track of changes in how gramr is storing and updating data in its database
  *  be extremely careful when performing requests as to avoid
     *  inconsistent queries
        *  gramr may change ledger between or while Horizon is making queries
        *  multiple gramr instances may not be in perfect sync with each other
     *  performance problems with poorly designed queries (non-indexed, etc).
* operationally, this creates coupling between gramr and Horizon:
  * a traffic spike in Horizon may cause enough stress on gramr that it loses sync
  * it's hard to "scale out" Horizon

### Additional reading

An overview of how the data flows within gramr is [this data flow presentation](software/core-data-flow.pdf)
...
properly registered by updating its cursor so that gramr can safely
garbage collect data no longer needed.
By default gramr only keeps track of its internal state and will
...
gramr exposes a simple HTTP based API in particular the `tx` endpoint is used to submit transactions.
