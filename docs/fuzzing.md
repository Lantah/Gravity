...
fuzzer with gravity. Support for this is still preliminary but it has
...
perform well in area (a) and with simple modifications we made to gravity, (b)
too. However, we've spent more energy with (c) for the `tx` fuzz mode, modifying
gravity such that it is more deterministic -- caches are cleared and randomness
...
## Building an instrumented gravity
...
to `overlay` and then `make fuzz`; this will do the following:
  ...
  - Run `gravity gen-fuzz fuzz-testcases/fuzz$i.xdr` ten thousand times
  ...
  - Run `afl-fuzz` on `gravity fuzz`, using those corpus directories

If `gravity fuzz` (or `afl-fuzz`) produces output such as 'Warning: AFL++
...
by feeding it to an instance of `gravity fuzz` run by hand, elsewhere (in
particular, not fighting for control over tmpdirs with the fuzzer's
`gravity` instances). Often a fuzzer "crash" is just the subprocess hitting
...
`gravity` in a different directory with `--enable-asan`, or valgrind, in
...
the tests). Then you can run `afl-fuzz -m 500 -M main -t 250 -i min-testcases -o fuzz-findings ./gravity fuzz --ll ERROR --process-id 0 --mode=${FUZZER_MODE} @@` 
...
part of staging-tests", here are some directions I think we should take fuzzing:
  ...
  - Try limiting the instrumentation to `gravity` itself, not libsodium,
